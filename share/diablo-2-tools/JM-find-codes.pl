...
my $d2s_file_location = '';
open my $D2S_LOCATION_CONFIG_FILE, '</etc/diablo-2-tools/char-selected.conf' or die $!;
#XXX open the d2s file for understanding the JM{Location} codes.
my $d2s_to_open = <$D2S_LOCATION_CONFIG_FILE>;
close $D2S_LOCATION_CONFIG_FILE;
print("$d2s_to_open:\n");
...
for my $i(0 .. $#JM_locations){
  ...
  #XXX uncomment this if you only want to target stuff.
...
#=cut
  ...
  print("\n");
  #say($sugar_slice_hex);
  system('perl', '/usr/share/diablo-2-tools/print-item-location.pl', '0x' . $sugar_slice_hex);
  sleep(1);
  #exit(0);
}
