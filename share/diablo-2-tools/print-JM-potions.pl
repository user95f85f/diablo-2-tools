...
open my $D2S_LOCATION_CONFIG, '</etc/diablo-2-tools/char-selected.conf' or die $!;
my $d2s_to_open = <$D2S_LOCATION_CONFIG>;
close $D2S_LOCATION_CONFIG;
print("$d2s_to_open:\n");
...
for my $i(0 .. $#JM_locations){
  ...
  if(length($sugar_slice_hex) == 28){
    ...
    system('perl', '/usr/share/diablo-2-tools/stam2rejuv.pl', '0x' . $sugar_slice_hex, $JM_locations[$i]);
    #exit(0);
  }
}
print "\n";
