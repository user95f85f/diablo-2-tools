package main
import ("fmt"; "io/ioutil"; "strconv"; "os"; "strings"; "runtime")
var p = fmt.Println
...
func main(){
  ...
  if runtime.GOOS == "linux" {
    ...
    d2s_file_location = strings.ReplaceAll(d2s_file_location, "/", "\\")
  }
  if len(d2s_file_location) == 0 {
    panic("Could not get d2s file location from /etc/diablo-2-tools/char-selected.conf")
  }
  dat, err := ioutil.ReadFile(d2s_file_location)
  check(err)
  oldFileSize := len(dat)
...
*/
  ...
  f, err := os.Create(d2s_file_location)
  check(err)
